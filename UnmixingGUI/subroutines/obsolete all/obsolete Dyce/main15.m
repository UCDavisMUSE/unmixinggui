function varargout = main15(varargin)
% MAIN15 M-file for main15.fig
%      MAIN15, by itself, creates a new MAIN15 or raises the existing
%      singleton*.
%
%      H = MAIN15 returns the handle to a new MAIN15 or the handle to
%      the existing singleton*.
%
%      MAIN15('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAIN15.M with the given input arguments.
%
%      MAIN15('Property','Value',...) creates a new MAIN15 or raises the
%      existing singleton*.  Starting from the leftFrame, property value pairs are
%      applied to the GUI before main15_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to main15_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help main15

% Last Modified by GUIDE v2.5 04-Aug-2008 18:10:14

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @main15_OpeningFcn, ...
                   'gui_OutputFcn',  @main15_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before main15 is made visible.
function main15_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to main15 (see VARARGIN)

% Choose default command line output for main15
...
handles.path = 'C:\Hillman_062607\DYCE Day 0\Mouse1\';
...
% UIWAIT makes main15 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = main15_OutputFcn(hObject, eventdata, handles) 
...
i = 1;
j = 0; % j will count how many organs we have selected. At the end of the loop, j will contain the number of organs
...
while (i <= 9)
    if (handles.organs(i).populated == 1)
        ...
        switch value
            case 1
                title = ['RCA applied on raw images, frames - ' num2str(timeSampled(1)) ' to ' num2str(timeSampled(end))];
...
%     colorPresentation(choosenOrgans,myDiff(handles.BW));
% 
% %% thresholding 
%     %choosenOrgansThresh = zeros(handles.N(1),handles.N(2),10);           
%     for i = 1:j
%         temp = choosenOrgans(:,:,i);
%         choosenOrgansThresh(:,:,i) = (temp > 0.4*max(temp(:)));
%     %   this is yet another attempt to threshold the organs
%     %   level = graythresh(temp);
%     %   choosenOrgans(ROI1,ROI2,j) = im2bw(temp,level);
%     end
%     assignin('base','choosenOrgansThresh',choosenOrgansThresh);
%     assignin('base','mouseEdge',handles.BW);
%     colorPresentation(choosenOrgansThresh,myDiff(handles.BW));
% end
...
while (i <= 9)
    if (handles.organs(i).populated == 1)
        ...
        for j = 10:10
            ...
            timeSampled = [frame1 + j : frame1 + RCAwidth - 1 + j];
            switch value
                case 1
                    myTitle = ['RCA applied on raw images, frames - ' num2str(timeSampled(1)) ' to ' num2str(timeSampled(end))];
                    [organList,ratio(j),snr(j),organImage] = rca_function(handles.x(ROI1,ROI2,timeSampled),componentsNumber,myTitle);
                case 2
            ...
            colorbar off;
            axis off image;
            title([num2str(frame1 + j) ' - ' num2str(frame1 + RCAwidth - 1 + j)]);
        end
        pos(1);
        plot(ratio);
        title('ratio');

...
%      applied to the GUI before main4_OpeningFunction gets called.  An
...
% Last Modified by GUIDE v2.5 30-Jun-2008 17:21:29
...
switch index
    case(1)
        ...
        for i = 1:handles.N(3)
            ...
            imwrite(0.25*handles.x(:,:,i),map,[handles.path 'tiff\raw\' name '_' num2str(i,'%03g') '.tif']);
        end
    case(2)
        ...
        for i = 1:handles.N(3)
            waitbar(i/handles.N(3),h)
            imwrite(scale(handles.deriv(:,:,i),n,-500,1600),map,[handles.path 'tiff\deriv\' name '_' num2str(i,'%03g') '.tif']);
        end
    case(3)
        ...
        for i = 1:handles.N(3)
            waitbar(i/handles.N(3),h)
            imwrite(scale(handles.deriv2(:,:,i),n,-500,1600),map,[handles.path 'tiff\deriv2\' name '_' num2str(i,'%03g') '.tif']);
        end
end
close(h)

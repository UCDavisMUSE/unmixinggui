function varargout = main18(varargin)
% MAIN18 M-file for main18.fig
%      MAIN18, by itself, creates a new MAIN18 or raises the existing
%      singleton*.
%
%      H = MAIN18 returns the handle to a new MAIN18 or the handle to
%      the existing singleton*.
%
%      MAIN18('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAIN18.M with the given input arguments.
%
%      MAIN18('Property','Value',...) creates a new MAIN18 or raises the
%      existing singleton*.  Starting from the leftFrame, property value pairs are
%      applied to the GUI before main18_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to main18_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help main18

% Last Modified by GUIDE v2.5 29-Aug-2008 11:45:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @main18_OpeningFcn, ...
                   'gui_OutputFcn',  @main18_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before main18 is made visible.
function main18_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to main18 (see VARARGIN)

% Choose default command line output for main18
...
% not a perfect way to find mouse edge
...
function varargout = main18_OutputFcn(hObject, eventdata, handles) 
...
theText = ['Do you want to save data for the current mouse?'];
...
% hObject    handle to showRCAresults (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

ForPresentation(handles.organList, ['Selected RCA components for ' num2str(handles.mouse) ...
    ...
    if (handles.organs(i).populated == 1)
        ...
        switch value
            case 1
                ...
                organList = RcaFunction(handles.x(ROI1,ROI2,timeSampled), componentsNumber, choose);     
            case 2
                myTitle = ['RCA applied on derivative images, region - ' ...
                    handles.organs(i).name ', frames - ' num2str(timeSampled(1)) ' to ' num2str(timeSampled(end))];
                organList = RcaFunction(handles.deriv(ROI1,ROI2,timeSampled), componentsNumber, choose);
            case 3
                myTitle = ['deriv2, ' get(handles.deltaSpace,'String') 'x' get(handles.deltaTime,'String') ];
                organList = RcaFunction(handles.deriv2(ROI1,ROI2,timeSampled),componentsNumber, choose);
        end
        %close;
...
if value ~= 1
    theText = ['Seems you have already found the starting frame? Do you want to continue?'];
    button  = questdlg(textwrap({theText},25));
    if ~strcmp(button,'Yes')
       return;
    end
...
% %% saves the old mouse organ data
%  button = questdlg('Do you want to save data for the current mouse/blob?  .');
% if strcmp(button,'Yes')
%     saveOrgans_Callback(handles.saveOrgans, eventdata, handles);
% elseif strcmp(button,'Cancel')
%     return;

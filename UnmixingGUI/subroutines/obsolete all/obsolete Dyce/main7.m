...
%      applied to the GUI before main7_OpeningFunction gets called.  An
...
switch index
    case(1)
        if get(handles.scale,'Value') == 1
            ...
            image(scale(handles.x(:,:,currentSlide),handles.mapSize,minScale,maxScale));
        end
    case(2)
        if get(handles.scale,'Value') == 1
            imagesc(handles.deriv(:,:,currentSlide));
        else
            image(scale(handles.deriv(:,:,currentSlide),handles.mapSize,minScale,maxScale));
        end
    case(3)
        if get(handles.scale,'Value') == 1
            imagesc(handles.deriv2(:,:,currentSlide));
        else
            image(scale(handles.deriv2(:,:,currentSlide),handles.mapSize,minScale,maxScale));
        end
end
...
s = [];
for i = 1:9
    str = [handles.organs(i) ' ' num2str(handles.organs(i).rect) '            p.' num2str(handles.organs(i).populated)];

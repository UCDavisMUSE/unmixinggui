function varargout = main32_wo_ASE(varargin)
% MAIN32_WO_ASE M-file for main32_wo_ASE.fig
%      MAIN32_WO_ASE, by itself, creates a new MAIN32_WO_ASE or raises the existing
%      singleton*.
%
%      H = MAIN32_WO_ASE returns the handle to a new MAIN32_WO_ASE or the handle to
%      the existing singleton*.
%
%      MAIN32_WO_ASE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAIN32_WO_ASE.M with the given input arguments.
%
%      MAIN32_WO_ASE('Property','Value',...) creates a new MAIN32_WO_ASE or raises the
%      existing singleton*.  Starting from the left, property value pairs
%      are
%      applied to the GUI before main32_wo_ASE_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to main32_wo_ASE_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help main32_wo_ASE

% Last Modified by GUIDE v2.5 12-Jan-2011 01:19:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @main32_wo_ASE_OpeningFcn, ...
                   'gui_OutputFcn',  @main32_wo_ASE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before main32_wo_ASE is made visible.
function main32_wo_ASE_OpeningFcn(hObject, eventdata, handles, varargin)
...
handles.markerColors = {[1, 0, 0],[0,1,0], [0,0,1], ...
    [255, 192, 0]/255, [238, 0, 238]/255, [150, 50, 0]/255, ... 
    [255, 182,179]/255, [0	206	209]/255, [1, 1, 1], [0 0 0], ...
    [0 0.5 0]};  % this last one is a tempSpectra color
handles.markerColorNames = {'Red','Green','Blue', ...
    'Yellow', 'Magenta', 'Brown', 'Pink', 'Cyan', 'White', 'Black', 'Greenish'};
...
% Choose default command line output for main32_wo_ASE
function varargout = main32_wo_ASE_OutputFcn(hObject, eventdata, handles)
...
if get(handles.showMarker,'Value')
         for j = 1:handles.maxNumberOfSpectra
             if handles.spectra(j).show
                 patch(handles.spectra(j).y, handles.spectra(j).x,'w','LineWidth',2,'EdgeColor',handles.spectra(j).color);
             end
         end
end 
...
% 5) means for saving the unmixed individual planes as well as the composite image. 
% TO BE DONE
% m = colormap(1000,1);
% THINK ABOUT IT
...
if strcmp(ext, '.jpg')
    for i = 1:handles.maxNumberOfSpectra
        if handles.spectra(i).show
            % here the image is always scaled ; % for transmision values are low, less then for example 10
...
%                 % this is fluorescence case
                imwrite(uint8(255*double(handles.spectra(i).component)/ ...
                    double(max(handles.spectra(i).component(:)))), ...
                    fullfile(pathname, [stripedName, '_' handles.spectra(i).name, '_scaled', ext]));
                    % fullfile(pathname, filename),'jpg','Bitdepth',8);
        end
    end   
    imwrite(uint8(255*double(handles.composite)/ ...
                    double(max(handles.composite(:)))), ...
                    fullfile(pathname, [stripedName, '_composite', ext]));
end % of jpg case

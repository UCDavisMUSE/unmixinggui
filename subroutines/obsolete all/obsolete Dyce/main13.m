function varargout = main13(varargin)
% MAIN13 M-file for main13.fig
%      MAIN13, by itself, creates a new MAIN13 or raises the existing
%      singleton*.
%
%      H = MAIN13 returns the handle to a new MAIN13 or the handle to
%      the existing singleton*.
%
%      MAIN13('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAIN13.M with the given input arguments.
%
%      MAIN13('Property','Value',...) creates a new MAIN13 or raises the
%      existing singleton*.  Starting from the leftFrame, property value pairs are
%      applied to the GUI before main13_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to main13_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help main13
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @main13_OpeningFcn, ...
                   'gui_OutputFcn',  @main13_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before main13 is made visible.
function main13_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to main13 (see VARARGIN)

% Choose default command line output for main13
...
% UIWAIT makes main13 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = main13_OutputFcn(hObject, eventdata, handles) 
...
handles.organs = main13_for_double_view(eventdata, handles);
guidata(hObject,handles);
...
while (i <= 9)
    if (handles.organs(i).populated == 1)
        ...
        for k = 1:size(choose,2)
            j = j + 1;
            temp = organList(:,:,choose(k));
            %this is an attemt to threshold an image 
            choosenOrgans(ROI1,ROI2,j) = temp;
            choosenOrgansThresh(ROI1,ROI2,j) = (temp > 0.4*max(temp(:)));

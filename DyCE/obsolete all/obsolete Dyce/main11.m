function varargout = main11(varargin)
% MAIN11 M-file for main11.fig
%      MAIN11, by itself, creates a new MAIN11 or raises the existing
%      singleton*.
%
%      H = MAIN11 returns the handle to a new MAIN11 or the handle to
%      the existing singleton*.
%
%      MAIN11('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MAIN11.M with the given input arguments.
%
%      MAIN11('Property','Value',...) creates a new MAIN11 or raises the
%      existing singleton*.  Starting from the leftFrame, property value pairs are
%      applied to the GUI before main11_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to main11_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help main11

% Last Modified by GUIDE v2.5 18-Jul-2008 16:39:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 0;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @main11_OpeningFcn, ...
                   'gui_OutputFcn',  @main11_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before main11 is made visible.
function main11_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to main11 (see VARARGIN)

% Choose default command line output for main11
...
% UIWAIT makes main11 wait for user response (see UIRESUME)
% uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = main11_OutputFcn(hObject, eventdata, handles) 
...
switch value
    case 1
        ...
        handles.organList = rca_function(handles.x(handles.ROI1,handles.ROI2,:),componentsNumber,title);
    case 2
        title = ['deriv, ' get(handles.deltaSpace,'String') 'x' get(handles.deltaTime,'String') ];
        handles.organList = rca_function(handles.deriv(handles.ROI1,handles.ROI2,:),componentsNumber,title);
    case 3
        title = ['deriv2, ' get(handles.deltaSpace,'String') 'x' get(handles.deltaTime,'String') ];
        handles.organList = rca_function(handles.deriv2(handles.ROI1,handles.ROI2,:),componentsNumber,title);
end
update_Callback(handles.update, eventdata, handles);

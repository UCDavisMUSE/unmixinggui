%
% Yes, that is why I want you to test and implement variable scaling (mag). 
% When the background is low and the signal is high, NO scaling is required. 
% A simple (locally relevant) subtraction is all that is necessary. 
% So, find a relationship between the relative intensities of signal vs. background 
% and the appropriate mag factor....
...
N = size(x);
organList = rca_function(x, 2, 'blobs');
% choose = inputdlg('Choose background frame:');
% choose = str2num(choose{1});
choose = 1;

% swap
temp = organList(:,:,1);
organList(:,:,1) = organList(:,:,choose);
organList(:,:,choose) = temp;      
temp = trueSpectra(:,1);
trueSpectra(:,1) = trueSpectra(:,choose);
trueSpectra(:,choose) = temp;      
...
coeffTrue = FindCoeff(trueSpectra, x, N, 1);
...
% 3. Overall background mask and spectrum are determined.
...
% Background mask is found by excluding organs masks that are first dilated (here by 3 pixels),
...
for j = 1:N(3)
    finalSpectra(j,1) = MyMean( x(:,:,j), mask(:,:,1) );
end
...
for i = 2:M(3)
    ...
    insideContour = FindInsideContour( mask(:,:,i), 6);
    Pos(4);
        ...
        for j = 1 : numberOfPoints
            ...
            if checked( XI(j), YI(j) ) == 0
                ...
                if SpectraDistinctiveEnough(spectrum, 20 - l )
                    ...
                    checked( XI(j), YI(j) ) = 1;
                    finalOrganSpectra(:,j) = outsideSpectrum;
%                     plot(spectrum, outsideSpectrum);
%                     close;
                    mag(j) = parameters(1);
...
%% Optimization using other cost function

[spectrum, parameters] = ComputePureSpectrum1( cube, mask, finalSpectra(:,2), finalSpectra(:,1), 1);


% Composite( FindCoeff( finalSpectra, x, N, 1), '10:20');
...
%% 
Pos(3); plot(NormalizeSpectra(finalSpectra)); hold on; plot(NormalizeSpectra(trueSpectra));
Composite( FindCoeff( finalSpectra, x, 1), '');
%Composite( FindCoeff( finalSpectra .* ( finalSpectra > 0), x, N, 2), 'truncated zero');
